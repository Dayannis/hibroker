...
if ( !function_exists('shieldgroup_need_page_title') ) {
	function shieldgroup_need_page_title() {
		return apply_filters('shieldgroup_filter_need_page_title', true);
	}
}
...
if ( !function_exists('shieldgroup_show_layout') ) {
	function shieldgroup_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('shieldgroup_get_logo_image') ) {
	function shieldgroup_get_logo_image($type='') {
		$logo_image = '';
		if (shieldgroup_is_on(shieldgroup_get_theme_option('logo_retina_enabled')) && shieldgroup_get_retina_multiplier(2) > 1)
			$logo_image = shieldgroup_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) {
			if (empty($type) && function_exists('the_custom_logo')) {
				...
				$logo_image = shieldgroup_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
			}
		}
		return shieldgroup_remove_protocol_from_url($logo_image, false);
	}
}

// Return header video (if set)
if ( !function_exists('shieldgroup_get_header_video') ) {
	function shieldgroup_get_header_video() {
		$video = '';
		if (apply_filters('shieldgroup_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (shieldgroup_check_theme_option('header_video')) {
				$video = shieldgroup_get_theme_option('header_video');
...
if ( !function_exists('shieldgroup_show_post_meta') ) {
	function shieldgroup_show_post_meta($args=array()) {

		if (is_single() && shieldgroup_is_off(shieldgroup_get_theme_option('show_post_meta'))) return ' ';	// Space is need!
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post categories
				if ($comp == 'categories') {
					$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('shieldgroup_filter_get_post_categories', '');
					if (!empty($cats)) {
						?>
						<span class="post_meta_item post_categories"><?php shieldgroup_show_layout($cats); ?></span>
						<?php
					}

				// Post date
				} else if ($comp == 'date') {
					$dt = apply_filters('shieldgroup_filter_get_post_date', shieldgroup_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post author
					...
					if ($author_id > 0) {
						...
						<a class="post_meta_item post_author" rel="author" href="<?php echo esc_url($author_link); ?>">
							<?php echo esc_html__('by','shieldgroup').' '. esc_html($author_name); ?>
						</a>
						<?php
					}
				...
				// Post counters
					...
					if ( ($output = shieldgroup_get_post_counters($args['counters'])) != '')
						shieldgroup_show_layout($output);
					else {
				...
				// Socials share
					...
					shieldgroup_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'shieldgroup'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'shieldgroup' ), '<span class="post_meta_item post_edit">', '</span>' );
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('shieldgroup_show_post_featured') ) {
	function shieldgroup_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => shieldgroup_get_theme_option('image_hover'),	// Hover effect
			...
			'seo' => shieldgroup_is_on(shieldgroup_get_theme_option('seo_snippets')),
		...
		$thumb_size = !empty($args['thumb_size']) 
						? $args['thumb_size'] 
						: shieldgroup_get_thumb_size(is_attachment() || is_single() ? 'full' : 'big');
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? shieldgroup_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = shieldgroup_get_post_image();
				if (!empty($image)) 
					$image = shieldgroup_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . shieldgroup_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured post_attachment<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>">
					...
					if (!$args['thumb_bg']) 
						echo wp_get_attachment_image( get_the_ID(), $thumb_size, false, 
													 shieldgroup_is_on(shieldgroup_get_theme_option('seo_snippets'))
				...
				?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$shieldgroup_attr = shieldgroup_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($shieldgroup_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($shieldgroup_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						shieldgroup_hovers_add_icons($args['hover']);
					}
					...
					if (($output=shieldgroup_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						shieldgroup_show_layout($output);
					...
					if (!empty($image)) {
						...
						shieldgroup_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = shieldgroup_get_post_video($post_content, false);
						if (empty($video))
							$video = shieldgroup_get_post_iframe($post_content, false);
						if (!empty($video)) {
							if ( $has_thumb ) {
								$video = shieldgroup_make_video_autoplay($video);
								?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
							}
							?><div class="post_video video_frame"><?php 
								if ( !$has_thumb ) {
									shieldgroup_show_layout($video);
								}
							?></div><?php
						}
					...
					// Put audio over the thumb
						...
						$audio = shieldgroup_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = shieldgroup_get_post_iframe($post_content, false);
						if (!empty($audio)) {
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
								// Add author and title
								$media_author = shieldgroup_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = shieldgroup_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !shieldgroup_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php shieldgroup_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !shieldgroup_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php shieldgroup_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								shieldgroup_show_layout($audio); 
							?></div><?php
						}
					}
				}
				...
				shieldgroup_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				shieldgroup_show_layout($post_info);
...
if ( !function_exists('shieldgroup_get_no_image') ) {
	function shieldgroup_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = shieldgroup_get_theme_option( 'no_image' );
			if (empty($img)) $img = shieldgroup_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('shieldgroup_add_bg_in_post_nav') ) {
	function shieldgroup_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = shieldgroup_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), shieldgroup_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), shieldgroup_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'shieldgroup-main', $css );
	}
}

// Show related posts
if ( !function_exists('shieldgroup_show_related_posts') ) {
	function shieldgroup_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
		...
		$taxonomy = $args['post_type'] == 'post' ? 'category' : shieldgroup_get_post_type_taxonomy();
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'shieldgroup');
				?></h3>
...
if ( !function_exists('shieldgroup_show_portfolio_posts') ) {
	function shieldgroup_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? shieldgroup_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = shieldgroup_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = shieldgroup_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the shieldgroup_ajax_get_posts action
if ( !function_exists( 'shieldgroup_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_shieldgroup_ajax_get_posts',			'shieldgroup_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_shieldgroup_ajax_get_posts',	'shieldgroup_ajax_get_posts_callback');
	function shieldgroup_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			shieldgroup_storage_set('blog_mode', 'blog');
			shieldgroup_storage_set('options_meta', get_post_meta($id, 'shieldgroup_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => shieldgroup_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'shieldgroup');
		}
...
// Show pagination
if ( !function_exists('shieldgroup_show_pagination') ) {
	function shieldgroup_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = shieldgroup_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'shieldgroup' ),
				'next_text' => esc_html__( '>', 'shieldgroup' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'shieldgroup' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'shieldgroup'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'shieldgroup') : esc_html__('Newest posts', 'shieldgroup') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'shieldgroup') : esc_html__('Older posts', 'shieldgroup'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'shieldgroup_single_comments_field' ) ) {
    function shieldgroup_single_comments_field( $args ) {
