...
if (!function_exists('shieldgroup_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'shieldgroup_hovers_theme_setup3', 3 );
	function shieldgroup_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		shieldgroup_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'shieldgroup'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'shieldgroup') ),
				"std" => 'fade',
				"options" => array(
					'default'		=> esc_html__('Fade',				'shieldgroup'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'shieldgroup'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'shieldgroup') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'shieldgroup'),
					'icon'	=> esc_html__('Icon',	'shieldgroup'),
					'icons'	=> esc_html__('Icons',	'shieldgroup'),
					'zoom'	=> esc_html__('Zoom',	'shieldgroup'),
					'fade'	=> esc_html__('Fade',	'shieldgroup'),
					'slide'	=> esc_html__('Slide',	'shieldgroup'),
					'pull'	=> esc_html__('Pull',	'shieldgroup'),
					'border'=> esc_html__('Border',	'shieldgroup')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('shieldgroup_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'shieldgroup_hovers_theme_setup9', 9 );
	function shieldgroup_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'shieldgroup_hovers_frontend_scripts', 1010 );
		add_filter( 'shieldgroup_filter_localize_script','shieldgroup_hovers_localize_script' );
		add_filter( 'shieldgroup_filter_merge_scripts',	'shieldgroup_hovers_merge_scripts' );
		add_filter( 'shieldgroup_filter_merge_styles',	'shieldgroup_hovers_merge_styles' );
		add_filter( 'shieldgroup_filter_get_css', 		'shieldgroup_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'shieldgroup_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'shieldgroup_hovers_frontend_scripts', 1010 );
	function shieldgroup_hovers_frontend_scripts() {
		if ( shieldgroup_is_on(shieldgroup_get_theme_option('debug_mode')) && shieldgroup_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'shieldgroup-hovers', shieldgroup_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( shieldgroup_is_on(shieldgroup_get_theme_option('debug_mode')) && shieldgroup_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'shieldgroup-hovers',  shieldgroup_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('shieldgroup_hovers_merge_scripts')) {
	//Handler of the add_filter( 'shieldgroup_filter_merge_scripts', 'shieldgroup_hovers_merge_scripts' );
	function shieldgroup_hovers_merge_scripts($list) {
...
if (!function_exists('shieldgroup_hovers_merge_styles')) {
	//Handler of the add_filter( 'shieldgroup_filter_merge_styles', 'shieldgroup_hovers_merge_styles' );
	function shieldgroup_hovers_merge_styles($list) {
...
if (!function_exists('shieldgroup_hovers_localize_script')) {
	//Handler of the add_filter( 'shieldgroup_filter_localize_script','shieldgroup_hovers_localize_script' );
	function shieldgroup_hovers_localize_script($arr) {
		$arr['button_hover'] = shieldgroup_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('shieldgroup_hovers_add_icons') ) {
	function shieldgroup_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
					shieldgroup_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'shieldgroup') : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'shieldgroup');
						else
							esc_html_e('Details', 'shieldgroup');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-zoom-in"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'shieldgroup_hovers_get_css' ) ) {
	//Handler of the add_filter( 'shieldgroup_filter_get_css', 'shieldgroup_hovers_get_css', 10, 4 );
	function shieldgroup_hovers_get_css($css, $colors, $fonts, $scheme='') {
