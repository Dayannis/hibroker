...
if (!function_exists('shieldgroup_customizer_get_css')) {

	function shieldgroup_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = shieldgroup_customizer_add_theme_fonts($fonts);
...
CSS;
			$rez = apply_filters('shieldgroup_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
...
CSS;
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(shieldgroup_get_list_schemes()) : array($only_scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = shieldgroup_get_scheme_colors($scheme);
...
.shieldgroup_tabs .shieldgroup_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.shieldgroup_tabs .shieldgroup_tabs_titles li.ui-state-active a {
	color: {$colors['inverse_link']};
...
.post_info_item.post_info_counters  .post_counters_item {
    color: {$colors['text_hover']};
}
.post_meta  .post_meta_item.post_counters_item:hover,
.post_info_item.post_info_counters .post_counters_item:hover {
    color: {$colors['text_link']};
}
.widget .select_container select {
    background-color: {$colors['bg_color']} !important;
}
.vc_custom_1501497779238 h5 a:hover {
	color: {$colors['text_hover']};
}
CSS;
				
					$rez = apply_filters('shieldgroup_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'shieldgroup_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>