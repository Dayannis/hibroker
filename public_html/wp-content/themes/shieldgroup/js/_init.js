...
jQuery(document).ready(function() {
		...
		jQuery('.nav-links-more a').on('click', function(e) {
			...
			if (panel.length == 0) {															// Load simple page content
				...
				jQuery.post(SHIELDGROUP_STORAGE['ajax_url'], {
					...
					if (rez.error !== '') {
						panel.html('<div class="shieldgroup_error">'+rez.error+'</div>');
					} else {
	...
	function shieldgroup_init_media_elements(cont) {
		if (SHIELDGROUP_STORAGE['use_mediaelements'] && cont.find('audio:not(.inited),video:not(.inited)').length > 0) {
            if (window.mejs) {
				...
				setTimeout(function() { shieldgroup_init_media_elements(cont); }, 400);
		}
	}
	
	
	// Load the tab's content
	function shieldgroup_tabs_ajax_content_loader(panel, page, oldPanel) {
		if (panel.html().replace(/\s/g, '')=='') {
			var height = oldPanel === undefined ? panel.height() : oldPanel.height();
			if (isNaN(height) || height < 100) height = 100;
			panel.html('<div class="shieldgroup_tab_holder" style="min-height:'+height+'px;"></div>');
		} else
			panel.find('> *').addClass('shieldgroup_tab_content_remove');
		panel.data('need-content', false).addClass('shieldgroup_loading');
		jQuery.post(SHIELDGROUP_STORAGE['ajax_url'], {
			nonce: SHIELDGROUP_STORAGE['ajax_nonce'],
			action: 'shieldgroup_ajax_get_posts',
			blog_template: panel.data('blog-template'),
			...
			panel.removeClass('shieldgroup_loading');
			var rez = {};
			try {
				rez = JSON.parse(response);
			} catch (e) {
			...
			if (rez.error !== '') {
				...
				panel.prepend(rez.data).fadeIn(function() {
					...
					jQuery(window).trigger('scroll');
					setTimeout(function() {
						panel.find('.shieldgroup_tab_holder,.shieldgroup_tab_content_remove').remove();
						jQuery(window).trigger('scroll');
					}, 600);
				});
			}
	...
	// Comments form
	function shieldgroup_comments_validate(form) {
		form.find('input').removeClass('error_field');
		var comments_args = {
			error_message_text: SHIELDGROUP_STORAGE['strings']['error_global'],	// Global error message text (if don't write in checked field)
			error_message_show: true,									// Display or not error message
			error_message_time: 4000,									// Error message display time
			error_message_class: 'shieldgroup_messagebox shieldgroup_messagebox_style_error',	// Class appended to error message block
			error_fields_class: 'error_field',							// Class appended to error fields
			exit_after_first_error: false,								// Cancel validation and exit after first error
			rules: [
				{
					field: 'comment',
					min_length: { value: 1, message: SHIELDGROUP_STORAGE['strings']['text_empty'] },
					max_length: { value: SHIELDGROUP_STORAGE['comment_maxlength'], message: SHIELDGROUP_STORAGE['strings']['text_long']}
				}
			]
		};
		if (form.find('.comments_author input[aria-required="true"]').length > 0) {
			comments_args.rules.push(
				{
					field: 'author',
					min_length: { value: 1, message: SHIELDGROUP_STORAGE['strings']['name_empty']},
					max_length: { value: 60, message: SHIELDGROUP_STORAGE['strings']['name_long']}
				}
			);
		}
		if (form.find('.comments_email input[aria-required="true"]').length > 0) {
			comments_args.rules.push(
				{
					field: 'email',
					min_length: { value: 1, message: SHIELDGROUP_STORAGE['strings']['email_empty']},
					max_length: { value: 60, message: SHIELDGROUP_STORAGE['strings']['email_long']},
					mask: { value: SHIELDGROUP_STORAGE['email_mask'], message: SHIELDGROUP_STORAGE['strings']['email_not_valid']}
				}
			);
		}
		var error = shieldgroup_form_validate(form, comments_args);
		return !error;
	}
