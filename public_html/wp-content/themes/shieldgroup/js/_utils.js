...
(function(){
	...
	window.shieldgroup_replicate = function(str, num) {
		var rez = '';
		for (var i=0; i < num; i++) {
			rez += str;
		}
	...
	window.shieldgroup_prepare_macros = function(str) {
		return str
			.replace(/\{\{/g, "<i>")
			.replace(/\}\}/g, "</i>")
			.replace(/\(\(/g, "<b>")
			...
			.on('click', 'tbody .iColorPicker_moreColors', function(e){
				...
				if (thead.hasClass('more_colors')) {
					...
					while (rgb[0]<0xF || rgb[1]<0xF || rgb[2]<0xF) {
						...
						i++;
						out += '<td style="background-color:'+shieldgroup_components2hex(rgb[0]*16+rgb[0],rgb[1]*16+rgb[1],rgb[2]*16+rgb[2])+'">&nbsp;</td>';
						rgb[2]+=3;
						if (rgb[2]>0xF) {
							rgb[1]+=3;
							if (rgb[1]>(j===0 ? 6 : 0xF)) {
	...
	window.shieldgroup_get_listbox_selected_index = function(box) {
		for (var i = 0; i < box.options.length; i++) {
	...
	window.shieldgroup_get_listbox_selected_value = function(box) {
		for (var i = 0; i < box.options.length; i++) {
			if (box.options[i].selected) {
	...
	window.shieldgroup_get_listbox_selected_text = function(box) {
		for (var i = 0; i < box.options.length; i++) {
			if (box.options[i].selected) {
	...
	window.shieldgroup_get_listbox_selected_option = function(box) {
		for (var i = 0; i < box.options.length; i++) {
			if (box.options[i].selected) {
	...
	window.shieldgroup_get_radio_value = function(radioGroupObj) {
		for (var i=0; i < radioGroupObj.length; i++)
			if (radioGroupObj[i].checked) return radioGroupObj[i].value;
		return null;
	};
	
	window.shieldgroup_set_radio_checked_by_num = function(radioGroupObj, num) {
		for (var i=0; i < radioGroupObj.length; i++)
			if (radioGroupObj[i].checked && i!=num) radioGroupObj[i].checked=false;
			else if (i==num) radioGroupObj[i].checked=true;
	};
	
	window.shieldgroup_set_radio_checked_by_value = function(radioGroupObj, val) {
		for (var i=0; i < radioGroupObj.length; i++)
	...
	/*
	// Usage example:
	var error = shieldgroup_form_validate(jQuery(form_selector), {		// -------- Options ---------
	...
	*/
	...
	window.shieldgroup_form_validate = function(form, opt) {
		var error_msg = '';
		...
		if (obj!==null && (typeof(obj)=='object' || typeof(obj)=='array')) {
			for (var prop in obj) {
				...
				if (level<recursive && (typeof(obj[prop])=='object' || typeof(obj[prop])=='array') && obj[prop]!=obj)
					dispStr += addStr + prop + '=' + shieldgroup_debug_object(obj[prop], recursive, showMethods, level+1);
				else {
					try {
						curStr = "" + obj[prop];
					} catch (e) {
	...
	window.shieldgroup_debug_log = function(s, clr) {
		if (SHIELDGROUP_STORAGE['user_logged_in']) {
